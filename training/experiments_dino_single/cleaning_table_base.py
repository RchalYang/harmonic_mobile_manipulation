...
from training.tasks.cleaning_table import CleaningTableTaskSampler, CleaningTableTask
...
from training.tasks.table_filter import filter_dataset_cleaning_table
from prior.utils.types import DatasetDict

import copy

class CleaningTableBaseConfig(ExperimentConfig, ABC):
    """The base config for all CleaningTable experiments."""
    ...
    CLEANING_TABLE_TASK_TYPE = CleaningTableTask

    RUN_REAL = cfg.run_real

    @lazy_property
    def HOUSE_DATASET(self):
        return DatasetDict(
            train=filter_dataset_cleaning_table(prior.load_dataset("procthor-10k")["train"]),
            test=filter_dataset_cleaning_table(prior.load_dataset("procthor-10k")["test"]),
            val=filter_dataset_cleaning_table(prior.load_dataset("procthor-10k")["val"])
        )
    ...
    def make_sampler_fn(
        ...
        return CleaningTableTaskSampler(
            args=task_sampler_args, cleaning_table_task_type=cls.CLEANING_TABLE_TASK_TYPE
        )
    ...
    def _get_sampler_args_for_scene_split(
        ...
        return TaskSamplerArgs(
            ...
            reward_config=RewardConfig(**cfg.mdp.reward.cleaning_table[mode]),
    ...
    def train_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        # FIlter Only Once
        # train_houses = filter_dataset_cleaning_table(train_houses)
    ...
    def valid_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        # val_houses = filter_dataset_cleaning_table(val_houses)
        if self.TEST_ON_TRAINING:
            val_houses = self.HOUSE_DATASET["train"]
        extra_controller_args = {
            "reverse_at_boundary": cfg.mdp.reverse_at_boundary,
            "smaller_action": cfg.mdp.smaller_action,
            "his_len": cfg.model.his_len,
        ...
        get_logger().warning(extra_controller_args)
        out = self._get_sampler_args_for_scene_split(
            houses=random.choices(val_houses, k=100),
            mode="eval",
    ...
    def test_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        # test_houses = filter_dataset_cleaning_table(test_houses)
        extra_controller_args = {
            "reverse_at_boundary": cfg.mdp.reverse_at_boundary,
            "smaller_action": cfg.mdp.smaller_action,
            "his_len": cfg.model.his_len,
