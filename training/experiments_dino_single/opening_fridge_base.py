...
from training.tasks.opening_fridge import OpeningfridgeTaskSampler, OpeningfridgeTask
from training.utils.types import RewardConfig, TaskSamplerArgs
from training.robot import STRETCH_ENV_ARGS

from training.tasks.fridge_filter import filter_dataset_fridge, get_fridge_property
from prior.utils.types import DatasetDict

class OpeningfridgeBaseConfig(ExperimentConfig, ABC):
    """The base config for all Openingfridge experiments."""
    ...
    OPENING_fridge_TASK_TYPE = OpeningfridgeTask

    @lazy_property
    def HOUSE_DATASET(self):
        return DatasetDict(
            train=filter_dataset_fridge(prior.load_dataset("procthor-10k")["train"]),
            test=filter_dataset_fridge(prior.load_dataset("procthor-10k")["test"]),
            val=filter_dataset_fridge(prior.load_dataset("procthor-10k")["val"])
        )
    ...
    def _get_sampler_args_for_scene_split(
        ...
        return TaskSamplerArgs(
            ...
            reward_config=RewardConfig(**cfg.mdp.reward.opening_fridge[mode]),
    ...
    def valid_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        val_houses = filter_dataset(val_houses, close_fridge=False, init_openness=cfg.task_config["init_openness"])
        out = self._get_sampler_args_for_scene_split(
            houses=val_houses,
            mode="eval",
