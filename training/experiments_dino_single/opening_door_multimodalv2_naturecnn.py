...
from training.tasks.opening_door import (
    OpeningDoorGraspTask,
    OpeningDoorGraspTwoStageTask,
    OpeningDoorGraspIterativeTask,
    OpeningDoorNWGraspTask,
    OpeningDoorNWGraspTwoStageTask,
    OpeningDoorNWGraspIterativeTask,
    OpeningDoorTaskSampler,
)
...
from .opening_door_base import OpeningDoorBaseConfig
...
from ..models.ncamera_dino_ablation import (
    NoPretrainMultiModalNCameraActorCritic,
    NoPretrainModalPrevActNCameraActorCritic,
)
from ..models.ncamera_transformer import (
    SkillTransformerPrevActNCameraActorCritic
)
...
from training.preprocessors.nopretrain_aug_preprocessor import NoPretrainAugHistoryPreprocessor, NoPretrainAugPreprocessor

from allenact.base_abstractions.task import TaskSampler
from training.utils.types import RewardConfig, TaskSamplerArgs


def get_opening_door_task(task_string):
...
def get_nopretrain_preprocessor(
    preprocessor_type, cfg, sensor, output_uuid,
):
  # preprocessor_type = eval(preprocessor_name)
  if preprocessor_type is NoPretrainAugHistoryPreprocessor:
    return preprocessor_type(
        # augment_obs=not cfg.eval,
        # augment_obs=True,
  ...
  elif preprocessor_type is NoPretrainAugPreprocessor:
    return preprocessor_type(
        # augment_obs=True,
        augment_obs=not (cfg.eval and cfg.evaluation.no_eval_aug),
        rgb_input_uuid=sensor.uuid,
...
def get_model(
  ...
  model = model_type(
      ...
      # TODO set this more explicitly/finer-grained later. Eval decent proxy for now,
      visualize=cfg.eval,
...
class OpeningDoorGraspTwoStageFullMultiModalDinoViTPPOExperimentConfig(OpeningDoorBaseConfig):
  """An Object Navigation experiment configuration with RGB input."""

  # OPENING_DOOR_TASK_TYPE = OpeningDoorGraspTwoStageFullTask
  OPENING_DOOR_TASK_TYPE = eval(cfg.task.name)
...
#   DINO_MODEL = cfg.model.dino.model_type
  ...
  PREPROCESSOR_TYPE = NoPretrainAugHistoryPreprocessor if cfg.sensor.enable_history else NoPretrainAugPreprocessor

  MODEL_TYPE = cfg.model.model_type_name

  SENSORS = [
      get_RGB_sensor(NAVIGATION_CAM_SENSOR_TYPE, cfg, "rgb_lowres_navigation"),
      get_RGB_sensor(MANIPULATION_CAM_SENSOR_TYPE,
                     cfg, "rgb_lowres_manipulation"),
      get_proprio_sensor(PROPRIO_SENSOR_TYPE, cfg),
      TaskIdSensor(),
  ]

  vit_preprocessor_uuids = [
      "rgb_navigation_nopretrain",
      "rgb_manipulation_nopretrain"
  ]

  @classmethod
  def tag(cls):
    return "OpeningDoor-MultiModalV2-Augmented-NoPretrain-DDPPO"
  ...
  def preprocessors(self) -> Sequence[Union[Preprocessor, Builder[Preprocessor]]]:
    ...
    if rgb_sensor_navigation is not None:
      preprocessors.append(
          get_nopretrain_preprocessor(self.PREPROCESSOR_TYPE, cfg,
                                      rgb_sensor_navigation, "rgb_navigation_nopretrain")
      )

    rgb_sensor_manipulation = next(
        (s for s in self.SENSORS if isinstance(s, RGBSensorStretchControllerManipulationHist)
         or isinstance(s, RGBSensorStretchControllerManipulation)),
        None
    )
    if rgb_sensor_manipulation is not None:
      preprocessors.append(
          get_nopretrain_preprocessor(self.PREPROCESSOR_TYPE, cfg,
                                      rgb_sensor_manipulation, "rgb_manipulation_nopretrain")
      )
  ...
  def make_sampler_fn(
    ...
    if cfg.run_real:
      return RealOpeningDoorGraspTaskSampler(
  ...
  def create_model(self, **kwargs) -> nn.Module:
    ...
    vit_preprocessor_uuids = [
        s for s in self.vit_preprocessor_uuids if "_only_viz" not in s]

    model = get_model(
        self.MODEL_TYPE, cfg, self.OPENING_DOOR_TASK_TYPE, kwargs, vit_preprocessor_uuids
    )
    ...
    if (
      ...
      if not torch.cuda.is_available():
        model.load_state_dict(torch.load(
            cfg.checkpoint, map_location=torch.device('cpu'))["model_state_dict"])
      else:
