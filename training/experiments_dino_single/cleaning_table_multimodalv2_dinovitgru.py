...
from training.tasks.cleaning_table import (
    CleaningTableTask,
    CleaningTableTwoStageTask,
    CleaningTableSubgoalTask,
    CleaningTableIterativeTask,
    CleaningTableNWTask,
    CleaningTableNWTwoStageTask,
    # CleaningTableSubgoalTask,
    CleaningTableNWIterativeTask,
    CleaningTableTaskSampler,
)
...
from .cleaning_table_base import CleaningTableBaseConfig
...
def get_cleaning_table_task(task_string):
...
class CleanTableMultiModalDinoViTPPOExperimentConfig(CleaningTableBaseConfig):
    """An Object Navigation experiment configuration with RGB input."""

    CLEANING_TABLE_TASK_TYPE = eval(cfg.task.name)
    ...
    @classmethod
    def tag(cls):
        return "CleaningTable-MultiModalV2-Augmented-DinoViTGRU-DDPPO"
    ...
    def make_sampler_fn(
        ...
        if cfg.run_real:
            return RealCleaningTableTaskSampler(
    ...
    def create_model(self, **kwargs) -> nn.Module:
        ...
        model = get_model(
            self.MODEL_TYPE, cfg, self.CLEANING_TABLE_TASK_TYPE, kwargs, vit_preprocessor_uuids
        )
