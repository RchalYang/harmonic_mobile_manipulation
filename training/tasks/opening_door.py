...
from training.tasks.door_parsing import get_door_property
...
class OpeningDoorTask(Task[Controller]):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.door_id = task_info["door_id"]
        self.door_corner = task_info["door_corner"]
        self.door_width = task_info["door_width"]
        self.door_size = task_info["door_size"]
        ...
        self.door_reach_distance = self.task_info["door_reach_distance"]
        self.knob_reach_distance = self.task_info["knob_reach_distance"]
        self.opendoor_use_section_scale = self.task_info["opendoor_use_section_scale"]
    ...
    def get_opening_type(self):
        agent_center = np.squeeze(np.array([
        ...
        agent_to_door_axis = agent_center - self.door_corner
        product = np.sum(agent_to_door_axis * self.opened_direction)
        if  product > 0:
            self.opening_type = "Pull"
        else:
            self.opening_type = "Push"
    ...
    def open_door_according_to_action_pull(self, action, pre_move_info, post_move_info):
        ...
        openness_scale = 1 if not self.opendoor_use_section_scale else 1/(1 - np.clip(openness, 0, 0.9))
        ...
        pre_vec_to_door_axis = pre_ee_center - self.door_corner

        pre_angle = np.arccos(np.sum(pre_vec_to_door_axis * self.closed_direction) / (np.linalg.norm(pre_vec_to_door_axis) + 1e-5))
        pre_angle = 2 * pre_angle / np.pi

        # There suppose to be only one door and should be opennable
        current_door_angle = np.pi * 0.5 * openness
        openable_direction = np.cos(current_door_angle) * self.closed_direction + np.sin(current_door_angle) * self.opened_direction
        to_open_direction = np.cos(current_door_angle) * self.opened_direction - np.sin(current_door_angle) * self.closed_direction
        ...
        if np.sum(pre_vec_to_door_axis * self.opened_direction) < 0 or \
            np.sum(pre_vec_to_door_axis * self.closed_direction) < 0:
            return 0, openness_scale, end_effector_correct_position
    
        if pre_angle < openness:
            return 0, openness_scale, end_effector_correct_position
        
        if not self._reach_door:
            return 0, openness_scale, end_effector_correct_position
        ...
        if np.linalg.norm(pre_vec_to_door_axis) > self.door_width or np.linalg.norm(pre_vec_to_door_axis) < 0.5 * self.door_width:
            return 0, openness_scale, end_effector_correct_position


        pre_dis_along = np.sum(pre_vec_to_door_axis * openable_direction) / np.linalg.norm(pre_vec_to_door_axis) * openable_direction
        pre_dis_per = pre_vec_to_door_axis - pre_dis_along
        pre_dis_to_door = np.linalg.norm(pre_dis_per) - self.door_size

        if pre_dis_to_door > 0.4:
            return 0, openness_scale, end_effector_correct_position


        post_ee_center = post_move_info["end_effector_pos"]
        post_vec_to_door_axis = post_ee_center - self.door_corner

        post_angle = np.arccos(np.sum(post_vec_to_door_axis * self.closed_direction) / (np.linalg.norm(post_vec_to_door_axis) + 1e-5))
        ...
        post_dis_along = np.sum(post_vec_to_door_axis * post_openable_direction) / np.linalg.norm(post_vec_to_door_axis) * post_openable_direction
        post_dis_per = post_vec_to_door_axis - post_dis_along
        post_dis_to_door = np.linalg.norm(post_dis_per) - self.door_size

        if post_dis_to_door < 0.075:
            return 0, openness_scale, end_effector_correct_position

        post_agent_center = post_move_info["agent_pos"]
        post_agent_to_door_axis = post_agent_center - self.door_corner
        post_agent_along = np.sum(post_agent_to_door_axis * post_openable_direction) / np.linalg.norm(post_agent_to_door_axis) * post_openable_direction
        post_agent_per =  post_agent_to_door_axis - post_agent_along
        post_agent_dis_to_door = np.linalg.norm(post_agent_per) - self.door_size - 0.25

        if (post_agent_dis_to_door < post_dis_to_door or post_agent_dis_to_door < 0.05) and (
            np.linalg.norm(post_agent_to_door_axis) < self.door_size + 0.05
        ):
            return 0, openness_scale, end_effector_correct_position
        ...
        post_top_to_door_axis = post_top_center - self.door_corner
        post_top_along = np.sum(post_top_to_door_axis * post_openable_direction) / np.linalg.norm(post_top_to_door_axis) * post_openable_direction
        post_top_per = post_top_center - post_top_along
        post_top_dis_to_door = np.linalg.norm(post_top_per)  - self.door_size

        # print("Post Top Dis to Door:", post_top_dis_to_door)
        if post_top_dis_to_door < post_dis_to_door or post_top_dis_to_door < 0.075:
            return 0, openness_scale, end_effector_correct_position
    ...
    def open_door_according_to_action_push(self, action, mode=None):
        ...
        self.vec_to_door_axis = end_effector_center - self.door_corner

        angle = np.arccos(np.sum(self.vec_to_door_axis * self.closed_direction) / np.linalg.norm(self.vec_to_door_axis))
        angle = 2 * angle / np.pi

        if np.sum(self.vec_to_door_axis * self.opened_direction) < 0:
            angle = -angle
        ...
        to_close_direction = np.cos(current_angle) * self.closed_direction - np.sin(current_angle) * self.opened_direction
        ...
        self.project_vec_norm =  np.sum(end_effector_moving_direction * to_open_direction) /\
            (np.linalg.norm(end_effector_moving_direction) + 1e-5)
        self.to_close_project_vec_norm =  np.sum(end_effector_moving_direction * to_close_direction) /\
            (np.linalg.norm(end_effector_moving_direction) + 1e-5)

        dis_along = np.sum(self.vec_to_door_axis * openable_direction) / np.linalg.norm(self.vec_to_door_axis) * openable_direction
        dis_per = self.vec_to_door_axis - dis_along
        dis_to_door = np.linalg.norm(dis_per) - self.door_size

        if np.sum(self.vec_to_door_axis * openable_direction) < 0:
            return 0, openness_scale, end_effector_correct_position
        ...
        agent_to_door_axis = agent_pos - self.door_corner
        agent_along = np.sum(agent_to_door_axis * openable_direction) / np.linalg.norm(agent_to_door_axis) * openable_direction
        agent_per = agent_to_door_axis - agent_along
        ...
        if np.linalg.norm(self.vec_to_door_axis) > self.door_width or np.linalg.norm(self.vec_to_door_axis) < 0.5 * self.door_width:
            return 0, openness_scale, end_effector_correct_position
        ...
        if self.project_vec_norm > 0.6:
            ...
            if openness > 0.90:
                ...
                self._took_end_action = True
            return delta, openness_scale, end_effector_correct_position
        elif self.to_close_project_vec_norm > 0.6:
            ...
            self.stretch_controller.controller.step(
                action='OpenObject',
                objectId=self.door_id,
                openness=max(openness - DELTA_OPENNESS * 2, 0),
                forceAction=True
            )
            delta = openness - max(openness - DELTA_OPENNESS * 2, 0)
            openness = self.stretch_controller.controller.last_event.metadata["objects"][0]["openness"]
            self.max_openness = max(openness, self.max_openness)
            if openness > 0.90:
                self._success = True
    ...
    def _step(self, action: np.array) -> RLStepResult:
        ...
        self._reach_door = self.dist_to_target_func() <= self.door_reach_distance and \
        ...
        self._reach_knob = self.current_dist_to_knob <= self.knob_reach_distance and self._reach_door

        # Push 
        self.status_opening_push, self.push_openness_scale, ee_inplace_push = self.open_door_according_to_action_push(action)
        ...
        # Pull
        self.status_opening_pull, self.pull_openness_scale, ee_inplace_pull = self.open_door_according_to_action_pull(
    ...
    def get_knob_pos(self):
        knob_pos = self.stretch_controller.controller.step(
            action="GetDoorHandle",
            objectId=self.door_id
    ...
    def judge(self) -> float:
        ...
        too_close_to_door = self.dist_to_target_func() <= 0.5
        penalties += too_close_to_door * self.reward_config.too_close_penalty
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        metrics["progress_per_step"] = np.clip(self.max_openness / 0.85, 0, 1) / self.num_steps_taken()
        metrics["normalized_progress"] = np.clip(self.max_openness / 0.85, 0, 1)
...
class OpeningDoorIterativeTask(OpeningDoorTask):
    @property
    def action_space(self):
...
class OpeningDoorGraspTask(OpeningDoorTask):
    @property
    def action_space(self):
        return gym.spaces.Box(low=-1, high=1, shape=(6,))
    ...
    def open_door_according_to_action_pull(self, action, pre_move_info, post_move_info):
        ...
        pre_angle = np.arccos(np.sum(pre_vec_to_door_axis * self.closed_direction) / np.linalg.norm(pre_vec_to_door_axis))
        pre_angle = 2 * pre_angle / np.pi
        ...
        post_angle = np.arccos(np.sum(post_vec_to_door_axis * self.closed_direction) / np.linalg.norm(post_vec_to_door_axis))
        post_angle = 2 * post_angle / np.pi
        ...
        # print("Post Dis to Door:", post_dis_to_door)
        if post_dis_to_door < 0.075:
            return 0, openness_scale, end_effector_correct_position
        ...
        # print("Post Agent Dis to Door:", post_agent_dis_to_door)
        if post_agent_dis_to_door < post_dis_to_door or post_agent_dis_to_door < 0.05:
            return 0, openness_scale, end_effector_correct_position
    ...
    def open_door_according_to_action_push(self, action, mode=None):
        ...
        if self.project_vec_norm > 0.6:
            ...
            self.stretch_controller.controller.step(
                action='OpenObject',
                objectId=self.door_id,
                openness=min(openness - DELTA_OPENNESS, 0),
                forceAction=True
            )
            delta = -DELTA_OPENNESS
            openness = self.stretch_controller.controller.last_event.metadata["objects"][0]["openness"]
...
class OpeningDoorNWGraspTask(OpeningDoorGraspTask):
    @property
    def action_space(self):
        return gym.spaces.Box(low=-1, high=1, shape=(5,))
...
class OpeningDoorNWGraspIterativeTask(OpeningDoorNWGraspTask):
    @property
    def action_space(self):
        return gym.spaces.Box(low=-1, high=1, shape=(6,))
...
class OpeningDoorGraspIterativeTask(OpeningDoorGraspTask):
    @property
    def action_space(self):
...
class OpeningDoorGraspSubgoalTask(OpeningDoorGraspTask):
    @property
    def action_space(self):
        return gym.spaces.Box(low=-1, high=1, shape=(7,))
    ...
    def _step(self, action: np.array) -> RLStepResult:
        ...
        # Push 
        self.status_opening_push, self.push_openness_scale, ee_inplace_push = self.open_door_according_to_action_push_subgoal(
    ...
    def open_door_according_to_action_push_subgoal(self, action, mode=None):
        end_effector_correct_position = 0
        openness = self.stretch_controller.controller.last_event.metadata["objects"][0]["openness"]
...
class OpeningDoorGraspTwoStageTask(OpeningDoorGraspTask):
...
class OpeningDoorNWGraspTwoStageTask(OpeningDoorNWGraspTask):
    def __init__(self, **kwargs):
        super().__init__(**prepare_locals_for_super(locals()))
        # 0 for navigation
        # 1 for manipulation
...
class OpeningDoorGraspTwoStageHalfTask(OpeningDoorGraspTask):
    def __init__(self, **kwargs):
        super().__init__(**prepare_locals_for_super(locals()))
        # 0 for navigation
        # 1 for manipulation
...
class OpeningDoorGraspTwoStageFullTask(OpeningDoorGraspTask):
    def __init__(self, **kwargs):
        super().__init__(**prepare_locals_for_super(locals()))
        # 0 for navigation
        # 1 for manipulation
...
class OpeningDoorGraspSigmoidTask(OpeningDoorGraspTask):
    @property
    def action_space(self):
        return gym.spaces.Box(low=-1, high=1, shape=(7,))
...
class OpeningDoorASCTask(OpeningDoorTask):
    @property
    def action_space(self):
        return gym.spaces.Box(low=-1, high=1, shape=(7,))
...
class OpeningDoorRealTask(OpeningDoorTask):
...
class OpeningDoorTaskSampler(TaskSampler):
    def __init__(
        self,
        args: TaskSamplerArgs,
        opening_door_task_type: Type = OpeningDoorTask,
    ) -> None:
        self.args = args
        # self.rescale_rewards()
        self.opening_door_task_type = opening_door_task_type
        ...
        self._last_sampled_task: Optional[OpeningDoorTask] = None
    ...
    @property
    def last_sampled_task(self) -> Optional[OpeningDoorTask]:
    ...
    def filter_starting_positions_by_distance(self, door_pos, reachable_positions, door_property):
        # NOTE: may return -1 if the object is unreachable.
        filtered_reachable_positions = []
        filtered_reachable_positions_type = {
            "Pull": [],
            "Push": []
        }
        spawn_max = self.spawn_range_max + self.spawn_range_curriculum_step * self.episode_index
        for pos in reachable_positions:
            ...
            opening_type = self.get_opening_type(door_pos, pos, door_property)
            # get_logger().warning(opening_type)
            if opening_type in self.sample_opening_type:
                # continue
                if dist <= spawn_max and dist >= self.spawn_range_min:
                # if dist <= self.spawn_range_max:
                    filtered_reachable_positions_type[opening_type].append(pos)

        # Balance Sampling            
        if len(self.sample_opening_type) == 2:
            min_number = min(
                len(filtered_reachable_positions_type["Push"]),
                len(filtered_reachable_positions_type["Pull"]),
            )
            if min_number > 0:
                if len(filtered_reachable_positions_type["Push"]) > min_number:
                    filtered_reachable_positions_type["Push"] = random.sample(filtered_reachable_positions_type["Push"], min_number)
                if len(filtered_reachable_positions_type["Pull"]) > min_number:
                    filtered_reachable_positions_type["Pull"] = random.sample(filtered_reachable_positions_type["Pull"], min_number)
            
        filtered_reachable_positions = filtered_reachable_positions_type["Pull"] + filtered_reachable_positions_type["Push"]
        get_logger().warning(f"# Reachable Pull Position: {len(filtered_reachable_positions_type['Pull'])} in House # {self.house_index}")
        get_logger().warning(f"# Reachable Push Position: {len(filtered_reachable_positions_type['Push'])} in House # {self.house_index}")
        return filtered_reachable_positions, len(filtered_reachable_positions) > 0

    def get_opening_type(self, door_pos, pos, door_property):
        agent_center = np.squeeze(np.array([
                pos["x"],
                pos["z"]
        ]))
        agent_to_door_axis = agent_center - door_property["door_corner"]
        product = np.sum(agent_to_door_axis * door_property["opened_direction"])
        if  product > 0:
            return "Pull"
        else:
            return "Push"
    ...
    def increment_scene(self) -> bool:
        ...
        self.door_property = get_door_property(self.house)
        ...
        starting_positions, success = self.filter_starting_positions_by_distance(door_pos, reachable_positions, self.door_property)
        get_logger().warning(f"# Reachable Position: {len(starting_positions)} in House # {self.house_index}")
    ...
    def close_all_doors(self, door_ids, init_openness):
    ...
    def next_task(self, force_advance_scene: bool = False) -> Optional[OpeningDoorTask]:
        ...
        # self.door_property = get_door_property(self.house)
        # event = self.stretch_controller.reset(self.house)
        # get_logger().warning(f"NEXT TASK: Controller Reset {event.metadata['actionReturn']}")
        # get_logger().warning(f"HOUSE INFO: {self.house}")
        ...
        # size = np.min([
        #     self.stretch_controller.controller.last_event.metadata["objects"][0]["axisAlignedBoundingBox"]["size"]["x"],
        #     self.stretch_controller.controller.last_event.metadata["objects"][0]["axisAlignedBoundingBox"]["size"]["y"],
        #     self.stretch_controller.controller.last_event.metadata["objects"][0]["axisAlignedBoundingBox"]["size"]["z"]
        # ]) / 2
        door_size = 0.025
        # print(size, closed_direction, opened_direction)
        # exit()
        self.door_property["door_corner"] = self.door_property["door_corner"] + door_size * self.door_property["closed_direction"]
        self.door_property["door_width"] = self.door_property["door_width"] - door_size
        self.door_property["door_size"] = door_size


        self.close_all_doors(door_ids, init_openness=self.args.extra_task_spec["init_openness"])
        ...
        task_id = (f'opening_door_{target_object_type}'
        ...
        self._last_sampled_task = self.opening_door_task_type(
            ...
            task_info={
                ...
                "id": task_id,
                "door_reach_distance": self.args.extra_task_spec["door_reach_distance"],
                "knob_reach_distance": self.current_knob_reach_distance,
                "opendoor_use_section_scale": self.args.extra_task_spec["opendoor_use_section_scale"],
...
def get_fake_door_property():
    return {
        "door_id": "RealDoor",
        "door_corner": np.array([0,0]),
        "door_width": 1.0,
        "door_size": 0.025,
        "closed_direction": np.array([0,0]),
        "opened_direction": np.array([0,0]),
        "door_closed_center":  np.array([0,0])
    }

class RealOpeningDoorTaskSampler(OpeningDoorTaskSampler):
    def __init__(self, args: TaskSamplerArgs, opening_door_task_type: Type = OpeningDoorRealTask) -> None:
        super().__init__(args, opening_door_task_type)
        assert cfg.eval
        ...
        self.args.controller_args = {
            ...
            "port":50051, "width": 1280, "height": 720,
            "reverse_at_boundary": cfg.mdp.reverse_at_boundary,
            "smaller_action": cfg.mdp.smaller_action,
            "arm_y_range": (0.5, 1.1)
        }

        self.object_set = ['Doorway'] # base set - robothor and # self.args.target_object_types
    ...
    def next_task(self) -> Optional[OpeningDoorRealTask]:
        if self.args.max_tasks <= 0:
            return None
        ...
        while True:
            ...
            self._last_sampled_task = self.opening_door_task_type(
                ...
                task_info={
                    ...
                    "mirrored": False,
                    "id": task_id,
                    "door_reach_distance": 0,
                    "knob_reach_distance": 0,
                    "opendoor_use_section_scale": 0,
                    "init_openness": 0,
                    **get_fake_door_property()
                },
            )
            self.real_object_index += 1
            if self.real_object_index == len(self.object_set):
