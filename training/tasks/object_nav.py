...
class ObjectNavTask(Task[Controller]):
    def __init__(
        ...
        **kwargs,
        ...
        # get_logger().warning(self.sensor_suite)
        # get_logger().warning(self.sensor_suite.sensors)
        self.observations = [
            np.concatenate([
    ...
    def _step(self, action: np.array) -> RLStepResult:
        ...
        self.task_info["taken_actions"].append(str(action))

        # get_logger().warning(str(action))
        ### for the Kiana Experiment (tm)

        # # self._success = self._is_goal_in_range()
        self._success = self.dist_to_target_func() <= 0.5 and \
        ...
        #     self.task_info["action_successes"].append(False)
        # else:
        self.stretch_controller.continuous_agent_step(
            action
        )
        self.last_action_success = bool(self.stretch_controller.controller.last_event)
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        metrics["reach_target"] = self._success
...
class ObjectNavTaskSampler(TaskSampler):
    ...
    def next_task(self, force_advance_scene: bool = False) -> Optional[ObjectNavTask]:
        ...
        # get_logger().warning("Sample New Task")
