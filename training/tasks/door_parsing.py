...
def filter_dataset(dataset, close_door=False, init_openness=0):
    single_room_list = []
    ...
    for idx in single_room_list:
        ...
        for i in range(len(data_sample["doors"])):
            if "openable" not in data_sample["doors"][i] or \
                ...
                continue
            if close_door:
                data_sample["doors"][i]["openness"] = init_openness
...
def get_door_property(scene):
    ...
    return {
        ...
        "door_closed_center":  door_corner + 0.5 * (door["holePolygon"][1]["x"] - door["holePolygon"][0]["x"]) * direction
    }
