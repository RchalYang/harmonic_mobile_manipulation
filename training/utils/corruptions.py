### from https://github.com/allenai/robustnav/blob/main/allenact/base_abstractions/rgb_sensor_degradations.py
...
motion_blur_aug = iaa.MotionBlur(k=(3,9))
...
def clipped_zoom(x, severity=1.5): #NB: this zooms vertically only!
    img = np.array(x)

    h, w = img.shape[:2]
    zoom_tuple = (severity, 1+0*severity, 1)
            # Bounding box of the zoomed-in region within the input array
    zh = int(np.round(h / severity))
    zw = w# int(np.round(w / severity))
    top = (h - zh) // 2
    left = (w - zw) // 2

    out = scizoom(img[top:top+zh, left:left+zw], zoom_tuple, order=1)
...
def motion_blur(x, severity=1):
    ...
    x = np.array(x)
    x = motion_blur_aug.augment_image(x)

    if x.shape != (224, 224):
        return np.clip(x, 0, 255)
    else:  # greyscale to RGB
        return np.clip(np.array([x, x, x]), 0, 255)
