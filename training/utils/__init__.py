...
from training.utils.types import Vector3
...
def get_room_connections(house) -> Tuple[nx.Graph, Dict[int, int]]:
    ...
    for door in house["doors"]:
        ...
        door_room_1 = int(door["room1"].split("|")[-1]) if door["room1"] is not None else None
        if door_room_0 == door_room_1 or door_room_1 is None:
            continue
