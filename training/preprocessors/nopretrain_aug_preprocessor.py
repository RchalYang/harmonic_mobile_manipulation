...
class DinoViTEmbedder(nn.Module):
  ...
  def forward(self, x):
    assert x.shape[-2:] == (224,
                            224), f"Expected shape is 224x224; got {x.shape}"
    with torch.no_grad():
...
class NoPretrainAugPreprocessor(Preprocessor):
  DINO_RGB_MEANS = (0.485, 0.456, 0.406)
  DINO_RGB_STDS = (0.299, 0.224, 0.225)
...
class NoPretrainAugHistoryPreprocessor(NoPretrainAugPreprocessor):
  """Preprocess RGB or depth image using a ResNet model with CLIP model
  weights."""


  def process(self, obs: Dict[str, Any], *args: Any, **kwargs: Any) -> Any:
    x = obs[self.input_uuids[0]].to(self.device).permute(
        0, 1, 4, 2, 3
    )  # b[his]hwc -> b[his]chw
