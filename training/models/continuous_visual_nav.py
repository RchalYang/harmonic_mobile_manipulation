...
class LinearGaussianActorHead(nn.Module):
    def __init__(
        ...
        self.log_std = nn.Parameter(
            np.log(init_std) * torch.ones(num_outputs)
        )
    ...
    def forward(self, x: torch.FloatTensor):  # type: ignore
        ...
        logstd = torch.clamp(self.log_std, -3, 3)
        std = torch.exp(logstd)
        # noinspection PyArgumentList
        return GaussianDistr(mean, std)  # logits are [step, sampler, ...]


class LinearCriticSLHead(nn.Module):
    def __init__(self, input_size: int):
        super().__init__()
        # self.fc = nn.Sequential(
        #     nn.Linear(input_size, input_size),
...
class LinearGaussianActorSLHead(nn.Module):
    def __init__(
            self, num_inputs: int, num_outputs: int,
            add_tanh: bool,
            init_std: float,
...
class ContinuousVisualNavActorCritic(ActorCriticModel[GaussianDistr]):
    ...
    def create_state_encoders(
        ...
        #     input_size=int(add_prev_action_null_token) + self.action_space.shape[0],
